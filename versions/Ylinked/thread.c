...
/*
  ...
  //if( i->yeditor == 1 ){ printf("off. Disperse()-(%d, %d) - (%d,%d)\n", p->lat, p->lon, p->dne[k]->lat, p->dne[k]->lon); }
#if _DISTANCE
...
static void MateSearchMovement(pThreadData td, pIndividual i, pPatch p)
{
  ...
  //if( i->yeditor == 1 ){ printf("MateSearchMovement() of i[%d] age: %d, s: %d-(%d, %d) - (%d,%d)\n", i->yeditor, i->age, i->s, p->lat, p->lon, p->dne[k]->lat, p->dne[k]->lon); }
#if _DISTANCE
  RecordDistance(p->density, distance);
#endif
#endif
...
static void ExtensiveMateSearchMovement(pThreadData td, pIndividual i, pPatch p)
{
  ...
  //if( i->yeditor == 1 ){ printf("ExtensiveMateSearchMovement()-(%d, %d) - (%d,%d)\n", p->lat, p->lon, p->dne[k]->lat, p->dne[k]->lon); }
...
static void Mate(pThreadData td, pIndividual f, pIndividual m, pPatch p)
{
  ...
  //printf("in Mate()\n");
  if ( f->yeditor == 1 )                Error("!!*This ERROR shouldn't print, check Mate(), sterile female should get into this function\n");
  ...
  for(j=0; j<((int)(rn)); j++) {
      ...
      if( m->yeditor == 1){
        i->yeditor = 1;
        // printf("m: %d, f: %d, o[%d]: %d\n", m->yeditor, f->yeditor, i->s, i->yeditor);
      } else {
        i->yeditor = 0;
      }
      ...
      //if( Gen > 53 ){ printf("m: %d, f: %d, o[%d]: %d\n", m->yeditor, f->yeditor, i->s, i->yeditor); }
...
static void PolyMate(pThreadData td, pIndividual f, pIndividual m_one, pIndividual m_two, pPatch p)
{
  ...
  if ( f->yeditor == 1 )                Error("!!*This ERROR shouldn't print, check PolyMate(), sterile female should get into this function\n");
...
static void Inoculate(pThreadData td, pPatch p, int inocindividuals)
{
  ...
  //printf("inoc gen: %d [%d, %d], inocindividuals: %d\n", Gen, p->lat, p->lon, inocindividuals);
                ...
                if( Gen == InocGen[g] ) {
                    ...
                    if( td.Current[i][j].inocpatch == 1 ) {
                        ...
                        for(h=0, l=td.Current[i][j].ni; h<l; h++){
                            if( td.Current[i][j].i[h]->s == 0 && td.Current[i][j].i[h]->yeditor == 1 ) toxicmaleno++; // count toxic males
                        }
      ...
      for(i=0; i<WIDTH; i++){
          for(j=0; j<HEIGHT; j++){
              ...
              for(h=0 ; h<l; h++){          // loops through individuals in a patch
                  ...
                  //if( td.Current[i][j].i[h]->yeditor == 1 ){
                  //    printf("going through all individuals: sterile (%d) sex: (%d) at [%d, %d]\n", td.Current[i][j].i[h]->yeditor, td.Current[i][j].i[h]->s, i, j);
                  //}
                  //if( *(td.Current[i][j].i[h]->m0) == 1 || *(td.Current[i][j].i[h]->m1) == 1 )
                  //  printf("%llu %llu\n", *(td.Current[i][j].i[h]->m0), *(td.Current[i][j].i[h]->m1));
                  
                  //if( Gen >= 54 ){
                  //    printf("[%d, %d] %d/%d, s:%d, tp: [%d]\n", i, j, h+1, l, td.Current[i][j].i[h]->s, td.Current[i][j].i[h]->yeditor);
                  //}
                  ...
                  if( td.Current[i][j].i[h]->yeditor == 1 ){
                      //printf("this skips the sterile (%d) female (%d)  at [%d, %d]\n", td.Current[i][j].i[h]->yeditor, td.Current[i][j].i[h]->s, i, j);
                      continue; // Skip any sterile females
                  }
                  //printf("here\n");
                  ...
                  //printf("**found mating female [%d] with m:(%d, %d), (at %d, %d), tp: %d \n", td.Current[i][j].i[h]->s, *(td.Current[i][j].i[h]->m0), *(td.Current[i][j].i[h]->m1), i, j, td.Current[i][j].i[h]->yeditor);
                  ...
                  if( polysuccess == 1 ) {
                      ...
                      //printf("S1-PM\n");
                      //if( matingfemale->yeditor == 1 )
                      //    printf("PMate-female (%llu) yeditor [%d]; 1st male: (%llu) %d/%d; 2nd male: (%llu) (%d/%d)\n", matingfemale, matingfemale->yeditor, matingmale, *(matingmale->m0),*(matingmale->m1), secondmatingmale, *(secondmatingmale->m0),*(secondmatingmale->m1));
                      polymating++;
                  }
                  ...
                  if( polysuccess == 0 ){
                      if( success != 0 ){
                          //printf("1 mating female (%d, %d) ind's sex: %d, tp: %d \n", i, j, td.Current[i][j].i[h]->s, td.Current[i][j].i[h]->yeditor);
                  ...
                  if( !maleprobsum || !numberofmales ){
                      ...
                      if( success != 0 ){
                          ...
                          //printf("S2-M\n");
                          //if( matingfemale->yeditor == 1 )
                              //printf("S2-female (%llu) yeditor [%d], male (%llu) is (%d/%d)\n", matingfemale, matingfemale->yeditor, matingmale, *(matingmale->m0),*(matingmale->m1));
                          mating++;
                      }
                  }
                      ...
                      for( matedist=1; matedist<((_DRANGE-1)/2+1); matedist++ ){
                          ...
                          for(mg=0; mg < (nPatches); mg++ ){
                              ...
                              if( success != 0 && distmatingsuccess != 1){
                                  ...
                                  //printf("S3-M\n");
                                  //if( matingfemale->yeditor == 1 )
                                  //    printf("S3-female (%llu) yeditor [%d], male (%llu) is (%d/%d)\n", matingfemale, matingfemale->yeditor, matingmale, *(matingmale->m0),*(matingmale->m1));
                                  distmatingsuccess = 1;
                                  mating++;
                                  break;
                              }
                          ...
                          if( !distmatingsuccess && roammate == 0){
                              for(mg=0; mg < (nPatches); mg++ ){
                                  ...
                                  if( matingfemale->yeditor == 1 ) break;
                                  //printf("S4-mating male (%llu): (%d/%d) \n", matingmale, *(matingmale->m0), *(matingmale->m1));
                                  
                                  if( success != 0 ){
                                      //printf("4 mating female (%d, %d) ind's sex: %d, tp: %d \n", i, j, matingfemale->s, matingfemale->yeditor);
                                      Mate(&td,matingfemale,matingmale,&td.Next[i][j]);
                                      roammate = 1;
                                      if( matingflagingeneration == 0 ) matingflagingeneration = 1;
                                      //printf("S4-M\n");
                                      //if( matingfemale->yeditor == 1 )
                                          //printf("S4-female (%llu) yeditor [%d], male (%llu) is %d/%d\n", matingfemale, matingfemale->yeditor, matingmale, *(matingmale->m0),*(matingmale->m1));
                                      distmatingsuccess = 1;
                                      mating++;
                                      break;
                                  }
