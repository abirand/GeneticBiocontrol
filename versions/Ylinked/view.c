...
int ReadSys(char *fn)
{
  ...
  for(i=0; i<nGen; i++) {
    ...
    if( fscanf(f, "%lf %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %lf %lf %d %d %d %d ",
                  &Gen[i].acp, &Gen[i].gMax, &Gen[i].rMax, &Gen[i].geno_ze, &Gen[i].geno_on,
                  &Gen[i].geno_tw, &Gen[i].geno_th, &Gen[i].geno_fo, &Gen[i].geno_fi, &Gen[i].geno_si, &Gen[i].geno_se, &Gen[i].geno_ei, &Gen[i].amc, &Gen[i].amk, &Gen[i].off, &Gen[i].mor, &Gen[i].mpat, &Gen[i].spat )  != 18 )
...
int ReadDeme(char *fn)
{
  ...
  for(i=0; i<nGen; i++) {
    ...
    if( fscanf(f, "%llu %llu ", &p->ip, &p->mp) != 2)
        { fprintf(stderr,"niche_view: deme: trait error (%llu,[%llu,%llu])\n",i,j,k); return 1; }
	/* Read average traits */
	//for(l=0; l<nTraits; l++)
...
void DrawTextStats(GLWindow *glw, float tx, float ty)
{
  ...
  y(); glRasterPos2f(120.0f,  40.0f); printGLf(glw->font,"%.2lf (%d/%llu)",((double)(Gen[Frame-1].g)-48)/((double)(MATINGCYCLES)), Frame-1, nGen-1);
  ...
  w(); glRasterPos2f(  0.0f,  80.0f); printGLf(glw->font,"N(max):");
  y(); glRasterPos2f(120.0f,  80.0f); printGLf(glw->font,"%llu",GlobalMax);
    
  //w(); glRasterPos2f(  0.0f, 100.0f); printGLf(glw->font,"T(reduction):");
  //y(); glRasterPos2f(120.0f, 100.0f); printGLf(glw->font,"%.2lf",((double)(TPopSizeReduction-14))/((double)(MATINGCYCLES)));

  w(); glRasterPos2f(  0.0f, 100.0f); printGLf(glw->font,"Multiple Paternity:");
  y(); glRasterPos2f(120.0f, 100.0f); printGLf(glw->font,"%.2lf",((double)(Gen[Frame-1].mpat))/((double)(Gen[Frame-1].spat+Gen[Frame-1].mpat)));
}
...
void DrawDemeGraph(GLWindow *glw, float tx, float ty)
{
  ...
  for(i=0; i<Width; i++) {
    for(j=0; j<Height; j++) {
      ...
      if( ((int)Gen[Frame-1].p[i][j].ip) == 1 ) glColor3ub(172,160,172);
      if( ((int)Gen[Frame-1].p[i][j].mp) == 1 ) glColor3ub(142,130,142);
...
//printf( "demesfig.eps file is opened!\n" );
  ...
  for(i=0; i<Width; i++) {
    for(j=0; j<Height; j++) {
      ...
      if( ((int)Gen[Frame-1].p[i][j].ip) == 1 ) PS_Color(f,  142/((float)255),130/((float)255),142/((float)255));
      //if( ((int)Gen[Frame-1].p[i][j].mp) == 1 ) PS_Color(f,  142/((float)255),130/((float)255),142/((float)255));
#else
...
void DrawSummaryGraph(GLWindow *glw, float tx, float ty)
{
  ...
  draw:
    ...
    glColor3ub( 78, 120, 181); glRasterPos2f(i+=40, 165);  printGLf(glw->font,"f. female");
    glColor3ub( 23, 155,  98); glRasterPos2f(i+=60, 165);  printGLf(glw->font,"inf. f");
    glColor3ub(184,  27,  74); glRasterPos2f(i+=65, 165);  printGLf(glw->font,"tg male");
    ...
    for(i=0; i<nGen; i++) {
      ...
      buckets[0][j] += Gen[i].acp; // tg. male
      buckets[1][j] += Gen[i].amc; // inf. female
      buckets[2][j] += Gen[i].amk; // fertile female
...
#else
    glColor3ub(100, 100, 100);  glRasterPos2f(i+=40, 165);  printGLf(glw->font,"wildtype");     // 0
    glColor3ub(197,  42,  81);  glRasterPos2f(i+=54, 165);  printGLf(glw->font,"transgenic (genotype freq.) ");   // 1
...
void DrawHoverBox(GLWindow *glw)
{
  ...
  if( Gen[Frame-1].p[i][j].ni ) {
    ...
    DrawOutlineTranslucentBox(0,0,180,80);
  ...
  if( !Gen[Frame-1].p[i][j].ni ) {
    ...
    glColor3ub(255,255,255);
    glRasterPos2f(15.0f,15.0f);
    printGLf(glw->font,"Patch popsize: %llu (f:%d, m:%d)", Gen[Frame-1].p[i][j].ni, ((int)(Gen[Frame-1].p[i][j].amf)), ((int)(Gen[Frame-1].p[i][j].amm)));
    glRasterPos2f(15.0f,35.0f);
    printGLf(glw->font,"[%llu, %llu]", i,j);

    glRasterPos2f(15.0f,55.0f);
    printGLf(glw->font,"tg males: %.3lf", Gen[Frame-1].p[i][j].m);
    glRasterPos2f(15.0f,75.0f);
    printGLf(glw->font,"inf. fem: %.3lf", Gen[Frame-1].p[i][j].f);
  }
}
...
static void DrawScene(GLWindow *glw)
{
  ...
  //DrawGenotypeHistogramI        (glw, Width*Scale+20.0f, 530.0f);
  //DrawGenotypeHistogramII       (glw, Width*Scale+20.0f, 630.0f);
  ...
  DrawGenerationSlider          (glw, Width*Scale+20.0f, 500.0f);

  DrawDVD                       (glw, 210.0f+NICHES*(20+HS), 460.0f+NICHES*(20+HS));
