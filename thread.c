...
static void Mate(pThreadData td, pIndividual f, pIndividual m, pPatch p)
{
  ...
  if( Gen == 53 ){ printf("in mating()\n"); }
    
  //printf("in Mate()\n");
  if ( *(m->m0) == 1 || *(m->m1) == 1 ) Error("!!*This ERROR shouldn't print, check Mate(), toxicmale should not get into this function\n");
  if ( f->toxicpregnancy == 1 )         Error("!!*This ERROR shouldn't print, check Mate(), toxicpregrantfemale should not get into this function\n");
...
static void PolyMate(pThreadData td, pIndividual f, pIndividual m_one, pIndividual m_two, pPatch p)
{
  ...
  if ( *(m_one->m0) == 1 || *(m_one->m1) == 1 ) Error("!!*This ERROR shouldn't print, check PolyMate(), toxic male not should get into this function\n");
  if ( *(m_two->m0) == 1 || *(m_two->m1) == 1 ) Error("!!*This ERROR shouldn't print, check PolyMate(), toxic male not should get into this function\n");
  if ( f->toxicpregnancy == 1 )         Error("!!*This ERROR shouldn't print, check PolyMate(), toxicpregnant should not get into this function\n");
   
  if( Gen == 53 ){ printf("in (poly)mating\n"); }
...
static void Inoculate(pThreadData td, pPatch p, int inocindividuals)
{
  ...
  for( j=0; j<inocindividuals; j++ ) {
      ...
      // this is trangenic genotype t1t0
      ...
      for(i=0; i<WIDTH; i++){
          for(j=0; j<HEIGHT; j++){
              for(h=0, l=td.Current[i][j].ni; h<l; h++){
                  ...
                  if( td.Current[i][j].i[h]->s == 1 && td.Current[i][j].i[h]->toxicpregnancy == 1 ){
                      Mortality++;
                      continue; // Skip toxic females
                  }
                  double randomno = 0.;
